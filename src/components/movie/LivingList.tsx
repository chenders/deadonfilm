...
export default function LivingList({ actors }: LivingListProps) {
  ...
  return (
    <div>
      <h2 className="font-display text-2xl text-green-800 mb-4">Living Cast Members</h2>
  ...
  return (
    <div className="bg-green-50 border border-green-200 rounded-lg p-4 flex gap-4 items-start">
      {profileUrl ? (
        ...
        <div className="w-16 h-20 rounded bg-green-100 flex items-center justify-center flex-shrink-0">
      ...
      <div className="flex-1 min-w-0">
        <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-1">
          <div>
            <h3 className="font-semibold text-green-900">{actor.name}</h3>
            <p className="text-sm text-green-700 italic">as {actor.character}</p>
          </div>

          {actor.age !== null && (
            <p className="text-sm text-green-600 font-medium">Age {actor.age}</p>
          )}
        </div>
      </div>
    </div>
