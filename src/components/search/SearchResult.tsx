...
function MediaPoster({
  ...
  return (
    <div className="-my-1 h-14 w-[38px] flex-shrink-0 overflow-hidden rounded bg-border-theme/10">
      {poster ? (
        <img
          src={poster.src}
        ...
        <div className="flex h-full w-full items-center justify-center text-foreground-muted/30">
...
export default function SearchResult({
  ...
  return (
    ...
    <li
      ...
      className={`cursor-pointer border-b border-border-theme/10 px-4 py-2 transition-colors last:border-b-0 ${isSelected ? "bg-surface-muted" : "hover:bg-surface-muted/50"}`}
      onClick={onSelect}
            ...
            <span
              ...
              className={`flex-shrink-0 rounded px-1.5 py-0.5 text-[10px] font-semibold uppercase tracking-wide ${
                result.media_type === "tv"
                  ? "bg-living/20 text-living-dark dark:bg-living/30 dark:text-living-light"
                  : "bg-border-theme/10 text-foreground-muted"
              }`}
            >
              {result.media_type === "tv" ? "TV" : "Film"}
            </span>
          </div>
          <div className="text-sm text-foreground-muted">{year}</div>
        </div>
