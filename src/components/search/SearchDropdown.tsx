...
export default function SearchDropdown({
  ...
  const baseClasses = "max-h-80 w-full overflow-y-auto bg-surface"
  const positionClasses = inline
    ? "" // Inline mode for modal - no extra positioning/borders
    : "absolute z-50 mt-1 rounded-lg border border-border-theme/30 shadow-lg" // Absolute mode for search bar
