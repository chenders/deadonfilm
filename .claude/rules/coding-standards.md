...
- **External links**: All links to external domains must use `target="_blank" rel="nofollow noopener noreferrer"`. `noopener` prevents the opened page from accessing `window.opener`. `noreferrer` prevents leaking the referrer URL. `nofollow` tells search engines not to pass SEO authority to external sites. **Exception**: editorially verified source links (where `sourceReliable` is true) may use `rel="noopener noreferrer"` without `nofollow` to signal E-E-A-T trust to search engines. This applies only to links whose source URL domain has been verified against `RELIABLE_DOMAINS` (ReliabilityTier >= 0.9).
