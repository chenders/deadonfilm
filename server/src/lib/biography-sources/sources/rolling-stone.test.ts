...
import { RollingStoneBiographySource } from "./rolling-stone.js"
...
function buildRollingStonePage(content: string, title = "John Wayne | Rolling Stone"): string {
  return `<html>
<head>
  <title>${title}</title>
...
describe("RollingStoneBiographySource", () => {
  let source: RollingStoneBiographySource

  beforeEach(() => {
    vi.clearAllMocks()
    source = new RollingStoneBiographySource()
  })
  ...
  describe("properties", () => {
    ...
    it("has correct type", () => {
      expect(source.type).toBe(BiographySourceType.ROLLING_STONE_BIO)
    })
  ...
  describe("lookup", () => {
    it("succeeds when web search returns Rolling Stone URL and page has content", async () => {
      const encodedUrl = encodeURIComponent(
        "https://www.rollingstone.com/movies/john-wayne-profile"
      )
          ...
          buildDuckDuckGoHtml([
            {
              ...
              title: "John Wayne Profile | Rolling Stone",
      ...
      mockFetchPage.mockResolvedValueOnce({
        content: buildRollingStonePage(richBiographicalContent),
        title: "John Wayne | Rolling Stone",
        url: "https://www.rollingstone.com/movies/john-wayne-profile",
        fetchMethod: "direct",
      })
      ...
      expect(result.data!.sourceName).toBe("Rolling Stone")
      expect(result.data!.sourceType).toBe(BiographySourceType.ROLLING_STONE_BIO)
      expect(result.data!.publication).toBe("Rolling Stone")
      expect(result.data!.domain).toBe("rollingstone.com")
    ...
    it("returns failure when no web search results found", async () => {
      ...
      expect(result.error).toContain("No Rolling Stone results found")
    })

    it("returns failure when page fetch fails", async () => {
      const encodedUrl = encodeURIComponent(
        "https://www.rollingstone.com/culture/john-wayne-profile"
      )
      ...
      mockFetchPage.mockResolvedValueOnce({
        content: "",
        title: "",
        url: "https://www.rollingstone.com/culture/john-wayne-profile",
        fetchMethod: "direct",
        error: "HTTP 401",
      })
    ...
    it("returns failure when cleaned content is too short", async () => {
      const encodedUrl = encodeURIComponent("https://www.rollingstone.com/movies/john-wayne-brief")
      ...
      mockFetchPage.mockResolvedValueOnce({
        ...
        title: "John Wayne",
        url: "https://www.rollingstone.com/movies/john-wayne-brief",
        fetchMethod: "direct",
      })
    ...
    it("sets correct source metadata on success", async () => {
      const encodedUrl = encodeURIComponent(
        "https://www.rollingstone.com/feature/john-wayne-profile"
      )
      ...
      mockFetchPage.mockResolvedValueOnce({
        content: buildRollingStonePage(richBiographicalContent),
        title: "John Wayne | Rolling Stone",
        url: "https://www.rollingstone.com/feature/john-wayne-profile",
        fetchMethod: "direct",
      })
      ...
      expect(result.success).toBe(true)
      expect(result.source.publication).toBe("Rolling Stone")
      expect(result.source.domain).toBe("rollingstone.com")
      expect(result.source.contentType).toBe("profile")
      expect(result.source.url).toContain("rollingstone.com")
