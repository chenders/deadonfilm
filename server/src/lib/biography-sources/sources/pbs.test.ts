...
import { PBSBiographySource } from "./pbs.js"
...
function buildPBSPage(content: string, title = "John Wayne | PBS"): string {
  return `<html>
<head>
  <title>${title}</title>
...
describe("PBSBiographySource", () => {
  let source: PBSBiographySource

  beforeEach(() => {
    vi.clearAllMocks()
    source = new PBSBiographySource()
  })
  ...
  describe("properties", () => {
    ...
    it("has correct type", () => {
      expect(source.type).toBe(BiographySourceType.PBS_BIO)
    })
  ...
  describe("lookup", () => {
    it("succeeds when web search returns PBS URL and page has content", async () => {
      const encodedUrl = encodeURIComponent("https://www.pbs.org/wgbh/john-wayne-profile")
      ...
      mockFetchPage.mockResolvedValueOnce({
        content: buildPBSPage(richBiographicalContent),
        title: "John Wayne | PBS",
        url: "https://www.pbs.org/wgbh/john-wayne-profile",
        fetchMethod: "direct",
      })
      ...
      expect(result.data!.sourceName).toBe("PBS")
      expect(result.data!.sourceType).toBe(BiographySourceType.PBS_BIO)
      expect(result.data!.publication).toBe("PBS")
      expect(result.data!.domain).toBe("pbs.org")
    ...
    it("returns failure when no web search results found", async () => {
      ...
      expect(result.error).toContain("No PBS results found")
    })

    it("returns failure when page fetch fails", async () => {
      const encodedUrl = encodeURIComponent("https://www.pbs.org/newshour/john-wayne-profile")
      ...
      mockFetchPage.mockResolvedValueOnce({
        content: "",
        title: "",
        url: "https://www.pbs.org/newshour/john-wayne-profile",
        fetchMethod: "direct",
        error: "HTTP 401",
      })
    ...
    it("returns failure when cleaned content is too short", async () => {
      const encodedUrl = encodeURIComponent("https://www.pbs.org/wgbh/john-wayne-brief")
      ...
      mockFetchPage.mockResolvedValueOnce({
        ...
        title: "John Wayne",
        url: "https://www.pbs.org/wgbh/john-wayne-brief",
        fetchMethod: "direct",
      })
    ...
    it("sets correct source metadata on success", async () => {
      const encodedUrl = encodeURIComponent("https://www.pbs.org/frontline/john-wayne-profile")
      ...
      mockFetchPage.mockResolvedValueOnce({
        content: buildPBSPage(richBiographicalContent),
        title: "John Wayne | PBS",
        url: "https://www.pbs.org/frontline/john-wayne-profile",
        fetchMethod: "direct",
      })
      ...
      expect(result.success).toBe(true)
      expect(result.source.publication).toBe("PBS")
      expect(result.source.domain).toBe("pbs.org")
      expect(result.source.contentType).toBe("profile")
      expect(result.source.url).toContain("pbs.org")
