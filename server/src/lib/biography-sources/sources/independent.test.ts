...
import { IndependentBiographySource } from "./independent.js"
...
function buildIndependentPage(content: string, title = "John Wayne | The Independent"): string {
  return `<html>
<head>
  <title>${title}</title>
...
describe("IndependentBiographySource", () => {
  let source: IndependentBiographySource

  beforeEach(() => {
    vi.clearAllMocks()
    source = new IndependentBiographySource()
  })
  ...
  describe("properties", () => {
    it("has correct name", () => {
      expect(source.name).toBe("The Independent")
    })

    it("has correct type", () => {
      expect(source.type).toBe(BiographySourceType.INDEPENDENT_BIO)
    })
  ...
  describe("lookup", () => {
    it("succeeds when web search returns Independent URL and page has content", async () => {
      const encodedUrl = encodeURIComponent(
        "https://www.independent.co.uk/arts-entertainment/john-wayne-profile"
      )
          ...
          buildDuckDuckGoHtml([
            {
              ...
              title: "John Wayne Profile | The Independent",
      ...
      mockFetchPage.mockResolvedValueOnce({
        content: buildIndependentPage(richBiographicalContent),
        title: "John Wayne | The Independent",
        url: "https://www.independent.co.uk/arts-entertainment/john-wayne-profile",
        fetchMethod: "direct",
      })
      ...
      expect(result.data!.sourceName).toBe("The Independent")
      expect(result.data!.sourceType).toBe(BiographySourceType.INDEPENDENT_BIO)
      expect(result.data!.publication).toBe("The Independent")
      expect(result.data!.domain).toBe("independent.co.uk")
    ...
    it("returns failure when no web search results found", async () => {
      ...
      expect(result.error).toContain("No The Independent results found")
    })
    ...
    it("returns failure when cleaned content is too short", async () => {
      const encodedUrl = encodeURIComponent(
        "https://www.independent.co.uk/arts-entertainment/john-wayne-brief"
      )
      ...
      mockFetchPage.mockResolvedValueOnce({
        ...
        title: "John Wayne",
        url: "https://www.independent.co.uk/arts-entertainment/john-wayne-brief",
        fetchMethod: "direct",
      })
    ...
    it("sets correct source metadata on success", async () => {
      const encodedUrl = encodeURIComponent("https://www.independent.co.uk/news/john-wayne-profile")
      ...
      mockFetchPage.mockResolvedValueOnce({
        content: buildIndependentPage(richBiographicalContent),
        title: "John Wayne | The Independent",
        url: "https://www.independent.co.uk/news/john-wayne-profile",
        fetchMethod: "direct",
      })
      ...
      expect(result.success).toBe(true)
      expect(result.source.publication).toBe("The Independent")
      expect(result.source.domain).toBe("independent.co.uk")
      expect(result.source.contentType).toBe("profile")
      expect(result.source.url).toContain("independent.co.uk")
