...
export class BBCNewsBiographySource extends BaseBiographySource {
  ...
  /**
   * Search DuckDuckGo HTML endpoint and return matching BBC URLs.
   */
  ...
  /**
   * Extract BBC URLs from DuckDuckGo HTML search results.
   * Matches both bbc.com and bbc.co.uk domains.
   */
  private extractUrlsFromDuckDuckGoHtml(html: string): string[] {
    const urls: string[] = []
    const isBbcUrl = (u: string) => u.includes("bbc.com") || u.includes("bbc.co.uk")
