...
import { NPRBiographySource } from "./npr.js"
...
function buildNPRPage(content: string, title = "John Wayne | NPR"): string {
  return `<html>
<head>
  <title>${title}</title>
...
describe("NPRBiographySource", () => {
  let source: NPRBiographySource

  beforeEach(() => {
    vi.clearAllMocks()
    source = new NPRBiographySource()
  })
  ...
  describe("properties", () => {
    it("has correct name", () => {
      expect(source.name).toBe("NPR")
    })

    it("has correct type", () => {
      expect(source.type).toBe(BiographySourceType.NPR_BIO)
    })
  ...
  describe("lookup", () => {
    it("succeeds when web search returns NPR URL and page has content", async () => {
      const encodedUrl = encodeURIComponent("https://www.npr.org/sections/john-wayne-profile")
      ...
      mockFetchPage.mockResolvedValueOnce({
        content: buildNPRPage(richBiographicalContent),
        title: "John Wayne | NPR",
        url: "https://www.npr.org/sections/john-wayne-profile",
        fetchMethod: "direct",
      })
      ...
      expect(result.data!.sourceName).toBe("NPR")
      expect(result.data!.sourceType).toBe(BiographySourceType.NPR_BIO)
      expect(result.data!.publication).toBe("NPR")
      expect(result.data!.domain).toBe("npr.org")
    ...
    it("returns failure when no web search results found", async () => {
      ...
      expect(result.error).toContain("No NPR results found")
    })
    ...
    it("returns failure when cleaned content is too short", async () => {
      const encodedUrl = encodeURIComponent("https://www.npr.org/sections/john-wayne-brief")
      ...
      mockFetchPage.mockResolvedValueOnce({
        ...
        title: "John Wayne",
        url: "https://www.npr.org/sections/john-wayne-brief",
        fetchMethod: "direct",
      })
    ...
    it("sets correct source metadata on success", async () => {
      const encodedUrl = encodeURIComponent("https://www.npr.org/movies/john-wayne-profile")
      ...
      mockFetchPage.mockResolvedValueOnce({
        content: buildNPRPage(richBiographicalContent),
        title: "John Wayne | NPR",
        url: "https://www.npr.org/movies/john-wayne-profile",
        fetchMethod: "direct",
      })
      ...
      expect(result.success).toBe(true)
      expect(result.source.publication).toBe("NPR")
      expect(result.source.domain).toBe("npr.org")
      expect(result.source.contentType).toBe("profile")
      expect(result.source.url).toContain("npr.org")
