/**
 * DuckDuckGo search source for biography information.
 *
 * Uses DuckDuckGo's HTML search endpoint (free, no API key required).
 * Extends BiographyWebSearchBase to support link following for richer results.
 *
 * Note: The HTML endpoint is deprecated and may block requests at any time.
 */
...
import { decodeHtmlEntities as decodeEntities } from "../../death-sources/html-utils.js"
 ...
 */
export class DuckDuckGoBiographySearch extends BiographyWebSearchBase {
  readonly name = "DuckDuckGo (Bio)"
  readonly type = BiographySourceType.DUCKDUCKGO_BIO
    ...
    while ((blockMatch = resultBlockRegex.exec(html)) !== null) {
      ...
      if (resultUrl && resultUrl.startsWith("http") && (title || snippet)) {
        results.push({
          ...
          url: resultUrl,
          snippet,
          source: BiographySourceType.DUCKDUCKGO_BIO as unknown as DataSourceType,
