...
import { WashingtonPostBiographySource } from "./washington-post.js"
...
function buildWaPoPage(content: string, title = "John Wayne | Washington Post"): string {
  return `<html>
<head>
  <title>${title}</title>
...
describe("WashingtonPostBiographySource", () => {
  let source: WashingtonPostBiographySource

  beforeEach(() => {
    vi.clearAllMocks()
    source = new WashingtonPostBiographySource()
  })
  ...
  describe("properties", () => {
    it("has correct name", () => {
      expect(source.name).toBe("Washington Post")
    })

    it("has correct type", () => {
      expect(source.type).toBe(BiographySourceType.WASHINGTON_POST_BIO)
    })
  ...
  describe("lookup", () => {
    it("succeeds when web search returns WaPo URL and page has content", async () => {
      const encodedUrl = encodeURIComponent(
        "https://www.washingtonpost.com/entertainment/john-wayne-profile"
      )
          ...
          buildDuckDuckGoHtml([
            {
              ...
              title: "John Wayne Profile | Washington Post",
      ...
      mockFetchPage.mockResolvedValueOnce({
        content: buildWaPoPage(richBiographicalContent),
        title: "John Wayne | Washington Post",
        url: "https://www.washingtonpost.com/entertainment/john-wayne-profile",
        fetchMethod: "direct",
      })
      ...
      expect(result.data!.sourceName).toBe("Washington Post")
      expect(result.data!.sourceType).toBe(BiographySourceType.WASHINGTON_POST_BIO)
      expect(result.data!.publication).toBe("Washington Post")
      expect(result.data!.domain).toBe("washingtonpost.com")
    ...
    it("returns failure when no web search results found", async () => {
      ...
      expect(result.error).toContain("No Washington Post results found")
    })
    ...
    it("returns failure when cleaned content is too short", async () => {
      const encodedUrl = encodeURIComponent(
        "https://www.washingtonpost.com/entertainment/john-wayne-brief"
      )
      ...
      mockFetchPage.mockResolvedValueOnce({
        ...
        title: "John Wayne",
        url: "https://www.washingtonpost.com/entertainment/john-wayne-brief",
        fetchMethod: "direct",
      })
    ...
    it("sets correct source metadata on success", async () => {
      const encodedUrl = encodeURIComponent(
        "https://www.washingtonpost.com/style/john-wayne-profile"
      )
      ...
      mockFetchPage.mockResolvedValueOnce({
        content: buildWaPoPage(richBiographicalContent),
        title: "John Wayne | Washington Post",
        url: "https://www.washingtonpost.com/style/john-wayne-profile",
        fetchMethod: "direct",
      })
      ...
      expect(result.success).toBe(true)
      expect(result.source.publication).toBe("Washington Post")
      expect(result.source.domain).toBe("washingtonpost.com")
      expect(result.source.contentType).toBe("profile")
      expect(result.source.url).toContain("washingtonpost.com")
