...
import { TimeBiographySource } from "./time.js"
...
function buildTimePage(content: string, title = "John Wayne | Time"): string {
  return `<html>
<head>
  <title>${title}</title>
...
describe("TimeBiographySource", () => {
  let source: TimeBiographySource

  beforeEach(() => {
    vi.clearAllMocks()
    source = new TimeBiographySource()
  })
  ...
  describe("properties", () => {
    it("has correct name", () => {
      expect(source.name).toBe("Time")
    })

    it("has correct type", () => {
      expect(source.type).toBe(BiographySourceType.TIME_BIO)
    })
  ...
  describe("lookup", () => {
    it("succeeds when web search returns Time URL and page has content", async () => {
      const encodedUrl = encodeURIComponent("https://time.com/entertainment/john-wayne-profile")
      ...
      mockFetchPage.mockResolvedValueOnce({
        content: buildTimePage(richBiographicalContent),
        title: "John Wayne | Time",
        url: "https://time.com/entertainment/john-wayne-profile",
        fetchMethod: "direct",
      })
      ...
      expect(result.data!.sourceName).toBe("Time")
      expect(result.data!.sourceType).toBe(BiographySourceType.TIME_BIO)
      expect(result.data!.publication).toBe("Time")
      expect(result.data!.domain).toBe("time.com")
    ...
    it("returns failure when no web search results found", async () => {
      ...
      expect(result.error).toContain("No Time results found")
    })

    it("returns failure when page fetch fails", async () => {
      const encodedUrl = encodeURIComponent("https://time.com/culture/john-wayne-profile")
      ...
      mockFetchPage.mockResolvedValueOnce({
        content: "",
        title: "",
        url: "https://time.com/culture/john-wayne-profile",
        fetchMethod: "direct",
        error: "HTTP 401",
      })
    ...
    it("returns failure when cleaned content is too short", async () => {
      const encodedUrl = encodeURIComponent("https://time.com/entertainment/john-wayne-brief")
      ...
      mockFetchPage.mockResolvedValueOnce({
        ...
        title: "John Wayne",
        url: "https://time.com/entertainment/john-wayne-brief",
        fetchMethod: "direct",
      })
    ...
    it("sets correct source metadata on success", async () => {
      ...
      expect(result.success).toBe(true)
      expect(result.source.publication).toBe("Time")
      expect(result.source.domain).toBe("time.com")
      expect(result.source.contentType).toBe("profile")
      expect(result.source.url).toContain("time.com")
