...
import { TelegraphBiographySource } from "./telegraph.js"
...
function buildTelegraphPage(content: string, title = "John Wayne | The Telegraph"): string {
  return `<html>
<head>
  <title>${title}</title>
...
describe("TelegraphBiographySource", () => {
  let source: TelegraphBiographySource

  beforeEach(() => {
    vi.clearAllMocks()
    source = new TelegraphBiographySource()
  })
  ...
  describe("properties", () => {
    it("has correct name", () => {
      expect(source.name).toBe("The Telegraph")
    })

    it("has correct type", () => {
      expect(source.type).toBe(BiographySourceType.TELEGRAPH_BIO)
    })
  ...
  describe("lookup", () => {
    it("succeeds when web search returns Telegraph URL and page has content", async () => {
      const encodedUrl = encodeURIComponent("https://www.telegraph.co.uk/films/john-wayne-profile")
          ...
          buildDuckDuckGoHtml([
            {
              ...
              title: "John Wayne Profile | The Telegraph",
      ...
      mockFetchPage.mockResolvedValueOnce({
        content: buildTelegraphPage(richBiographicalContent),
        title: "John Wayne | The Telegraph",
        url: "https://www.telegraph.co.uk/films/john-wayne-profile",
        fetchMethod: "direct",
      })
      ...
      expect(result.data!.sourceName).toBe("The Telegraph")
      expect(result.data!.sourceType).toBe(BiographySourceType.TELEGRAPH_BIO)
      expect(result.data!.publication).toBe("The Telegraph")
      expect(result.data!.domain).toBe("telegraph.co.uk")
    ...
    it("returns failure when no web search results found", async () => {
      ...
      expect(result.error).toContain("No Telegraph results found")
    })

    it("returns failure when page fetch fails", async () => {
      const encodedUrl = encodeURIComponent(
        "https://www.telegraph.co.uk/culture/john-wayne-profile"
      )
      ...
      mockFetchPage.mockResolvedValueOnce({
        content: "",
        title: "",
        url: "https://www.telegraph.co.uk/culture/john-wayne-profile",
        fetchMethod: "direct",
        error: "HTTP 401",
      })
    ...
    it("returns failure when cleaned content is too short", async () => {
      const encodedUrl = encodeURIComponent("https://www.telegraph.co.uk/films/john-wayne-brief")
      ...
      mockFetchPage.mockResolvedValueOnce({
        ...
        title: "John Wayne",
        url: "https://www.telegraph.co.uk/films/john-wayne-brief",
        fetchMethod: "direct",
      })
    ...
    it("sets correct source metadata on success", async () => {
      ...
      expect(result.success).toBe(true)
      expect(result.source.publication).toBe("The Telegraph")
      expect(result.source.domain).toBe("telegraph.co.uk")
      expect(result.source.contentType).toBe("profile")
      expect(result.source.url).toContain("telegraph.co.uk")
