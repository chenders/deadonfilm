...
import { ReutersBiographySource } from "./reuters.js"
...
function buildReutersPage(content: string, title = "John Wayne | Reuters"): string {
  return `<html>
<head>
  <title>${title}</title>
...
describe("ReutersBiographySource", () => {
  let source: ReutersBiographySource

  beforeEach(() => {
    vi.clearAllMocks()
    source = new ReutersBiographySource()
  })
  ...
  describe("properties", () => {
    ...
    it("has correct type", () => {
      expect(source.type).toBe(BiographySourceType.REUTERS_BIO)
    })
  ...
  describe("lookup", () => {
    it("succeeds when web search returns Reuters URL and page has content", async () => {
      const encodedUrl = encodeURIComponent("https://www.reuters.com/lifestyle/john-wayne-profile")
          ...
          buildDuckDuckGoHtml([
            {
              ...
              title: "John Wayne Profile | Reuters",
      ...
      mockFetchPage.mockResolvedValueOnce({
        content: buildReutersPage(richBiographicalContent),
        title: "John Wayne | Reuters",
        url: "https://www.reuters.com/lifestyle/john-wayne-profile",
        fetchMethod: "direct",
      })
      ...
      expect(result.data!.sourceName).toBe("Reuters")
      expect(result.data!.sourceType).toBe(BiographySourceType.REUTERS_BIO)
      expect(result.data!.publication).toBe("Reuters")
      expect(result.data!.domain).toBe("reuters.com")
    ...
    it("returns failure when no web search results found", async () => {
      ...
      expect(result.error).toContain("No Reuters results found")
    })

    it("returns failure when page fetch fails", async () => {
      const encodedUrl = encodeURIComponent("https://www.reuters.com/world/john-wayne-profile")
      ...
      mockFetchPage.mockResolvedValueOnce({
        content: "",
        title: "",
        url: "https://www.reuters.com/world/john-wayne-profile",
        fetchMethod: "direct",
        error: "HTTP 401",
      })
    ...
    it("returns failure when cleaned content is too short", async () => {
      const encodedUrl = encodeURIComponent("https://www.reuters.com/lifestyle/john-wayne-brief")
      ...
      mockFetchPage.mockResolvedValueOnce({
        ...
        title: "John Wayne",
        url: "https://www.reuters.com/lifestyle/john-wayne-brief",
        fetchMethod: "direct",
      })
    ...
    it("sets correct source metadata on success", async () => {
      ...
      expect(result.success).toBe(true)
      expect(result.source.publication).toBe("Reuters")
      expect(result.source.domain).toBe("reuters.com")
      expect(result.source.contentType).toBe("profile")
      expect(result.source.url).toContain("reuters.com")
