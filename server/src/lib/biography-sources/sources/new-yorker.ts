/**
 * The New Yorker biography source.
 *
 * Searches for actor profiles and biographical content on newyorker.com
 * via web search (Google CSE with DDG fallback), fetches with archive fallback
 * on block, and cleans the article.
 *
 * Reliability tier: TIER_1_NEWS (0.95) - authoritative literary magazine.
 */
...
export class NewYorkerBiographySource extends BaseBiographySource {
  readonly name = "The New Yorker"
  readonly type = BiographySourceType.NEW_YORKER_BIO
  ...
  protected async performLookup(actor: ActorForBiography): Promise<BiographyLookupResult> {
    ...
    // Step 1: Search for newyorker.com pages about this actor (Google CSE → DDG fallback)
    const query = `site:newyorker.com "${actor.name}" profile OR biography OR interview`
    const searchResult = await webSearch({
      query,
      domainFilter: "newyorker.com",
    })
    ...
    if (urls.length === 0) {
      return {
        ...
        source: this.createSourceEntry(startTime, 0, { queryUsed: query }),
        data: null,
        error: "No New Yorker results found via web search",
      }
    }
    ...
    if (pageResult.error || !pageResult.content) {
      return {
        ...
        data: null,
        error: `New Yorker page fetch failed: ${pageResult.error || "empty content"}`,
      }
    }
    ...
    if (text.length < MIN_CONTENT_LENGTH) {
      return {
        ...
        data: null,
        error: `New Yorker content too short (${text.length} chars, minimum ${MIN_CONTENT_LENGTH})`,
      }
    }
    ...
    // Step 7: Build result
    const articleTitle = metadata.title || pageResult.title || `${actor.name} - The New Yorker`

    const sourceData: RawBiographySourceData = {
      sourceName: "The New Yorker",
      sourceType: BiographySourceType.NEW_YORKER_BIO,
      text,
      url: targetUrl,
      confidence,
   ...
   * The New Yorker uses paths like /magazine/..., /culture/..., /news/..., /books/...
   */
