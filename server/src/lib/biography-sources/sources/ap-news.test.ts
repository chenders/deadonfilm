...
/**
 * Build an AP News-like HTML page with article content.
 */
function buildAPNewsPage(content: string, title = "John Wayne | AP News"): string {
  return `<html>
<head>
  <title>${title}</title>
...
describe("APNewsBiographySource", () => {
  let source: APNewsBiographySource

  beforeEach(() => {
    vi.clearAllMocks()
    source = new APNewsBiographySource()
  })
  ...
  describe("properties", () => {
    ...
    it("has correct type", () => {
      expect(source.type).toBe(BiographySourceType.AP_NEWS_BIO)
    })
  ...
  describe("lookup", () => {
    it("succeeds when DuckDuckGo returns AP News URL and page has content", async () => {
          ...
          buildDuckDuckGoHtml([
            {
              ...
              title: "John Wayne Profile | AP News",
      ...
      expect(result.data!.sourceName).toBe("AP News")
      expect(result.data!.sourceType).toBe(BiographySourceType.AP_NEWS_BIO)
      expect(result.data!.publication).toBe("Associated Press")
      expect(result.data!.domain).toBe("apnews.com")
    ...
    it("returns failure when AP News page fetch fails", async () => {
      const encodedUrl = encodeURIComponent("https://apnews.com/article/john-wayne-profile")
    ...
    it("returns failure when cleaned content is too short", async () => {
      const encodedUrl = encodeURIComponent("https://apnews.com/article/john-wayne-brief")
    ...
    it("sets correct source metadata on success", async () => {
      ...
      expect(result.success).toBe(true)
      expect(result.source.publication).toBe("Associated Press")
      expect(result.source.domain).toBe("apnews.com")
    ...
    it("prefers /article/ paths over other URLs", async () => {
      const articleUrl = "https://apnews.com/article/john-wayne-biography"
      const otherUrl = "https://apnews.com/hub/john-wayne"
