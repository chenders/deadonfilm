...
import { LATimesBiographySource } from "./la-times.js"
...
function buildLATimesPage(content: string, title = "John Wayne | Los Angeles Times"): string {
  return `<html>
<head>
  <title>${title}</title>
...
describe("LATimesBiographySource", () => {
  let source: LATimesBiographySource

  beforeEach(() => {
    vi.clearAllMocks()
    source = new LATimesBiographySource()
  })
  ...
  describe("properties", () => {
    it("has correct name", () => {
      expect(source.name).toBe("Los Angeles Times")
    })

    it("has correct type", () => {
      expect(source.type).toBe(BiographySourceType.LA_TIMES_BIO)
    })
  ...
  describe("lookup", () => {
    it("succeeds when web search returns LA Times URL and page has content", async () => {
      const encodedUrl = encodeURIComponent(
        "https://www.latimes.com/entertainment/john-wayne-profile"
      )
          ...
          buildDuckDuckGoHtml([
            {
              ...
              title: "John Wayne Profile | Los Angeles Times",
      ...
      mockFetchPage.mockResolvedValueOnce({
        content: buildLATimesPage(richBiographicalContent),
        title: "John Wayne | Los Angeles Times",
        url: "https://www.latimes.com/entertainment/john-wayne-profile",
        fetchMethod: "direct",
      })
      ...
      expect(result.data!.sourceName).toBe("Los Angeles Times")
      expect(result.data!.sourceType).toBe(BiographySourceType.LA_TIMES_BIO)
      expect(result.data!.publication).toBe("Los Angeles Times")
      expect(result.data!.domain).toBe("latimes.com")
    ...
    it("returns failure when no web search results found", async () => {
      ...
      expect(result.error).toContain("No Los Angeles Times results found")
    })
    ...
    it("returns failure when cleaned content is too short", async () => {
      const encodedUrl = encodeURIComponent(
        "https://www.latimes.com/entertainment/john-wayne-brief"
      )
      ...
      mockFetchPage.mockResolvedValueOnce({
        ...
        title: "John Wayne",
        url: "https://www.latimes.com/entertainment/john-wayne-brief",
        fetchMethod: "direct",
      })
    ...
    it("sets correct source metadata on success", async () => {
      const encodedUrl = encodeURIComponent("https://www.latimes.com/obituaries/john-wayne-profile")
      ...
      mockFetchPage.mockResolvedValueOnce({
        content: buildLATimesPage(richBiographicalContent),
        title: "John Wayne | Los Angeles Times",
        url: "https://www.latimes.com/obituaries/john-wayne-profile",
        fetchMethod: "direct",
      })
      ...
      expect(result.success).toBe(true)
      expect(result.source.publication).toBe("Los Angeles Times")
      expect(result.source.domain).toBe("latimes.com")
      expect(result.source.contentType).toBe("profile")
      expect(result.source.url).toContain("latimes.com")
