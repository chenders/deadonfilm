...
import { NewYorkerBiographySource } from "./new-yorker.js"
...
function buildNewYorkerPage(content: string, title = "John Wayne | The New Yorker"): string {
  return `<html>
<head>
  <title>${title}</title>
...
describe("NewYorkerBiographySource", () => {
  let source: NewYorkerBiographySource

  beforeEach(() => {
    vi.clearAllMocks()
    source = new NewYorkerBiographySource()
  })
  ...
  describe("properties", () => {
    it("has correct name", () => {
      expect(source.name).toBe("The New Yorker")
    })

    it("has correct type", () => {
      expect(source.type).toBe(BiographySourceType.NEW_YORKER_BIO)
    })
  ...
  describe("lookup", () => {
    it("succeeds when web search returns New Yorker URL and page has content", async () => {
      const encodedUrl = encodeURIComponent("https://www.newyorker.com/magazine/john-wayne-profile")
          ...
          buildDuckDuckGoHtml([
            {
              ...
              title: "John Wayne Profile | The New Yorker",
      ...
      mockFetchPage.mockResolvedValueOnce({
        content: buildNewYorkerPage(richBiographicalContent),
        title: "John Wayne | The New Yorker",
        url: "https://www.newyorker.com/magazine/john-wayne-profile",
        fetchMethod: "direct",
      })
      ...
      expect(result.data!.sourceName).toBe("The New Yorker")
      expect(result.data!.sourceType).toBe(BiographySourceType.NEW_YORKER_BIO)
      expect(result.data!.publication).toBe("The New Yorker")
      expect(result.data!.domain).toBe("newyorker.com")
    ...
    it("returns failure when no web search results found", async () => {
      ...
      expect(result.error).toContain("No New Yorker results found")
    })

    it("returns failure when page fetch fails", async () => {
      const encodedUrl = encodeURIComponent("https://www.newyorker.com/culture/john-wayne-profile")
      ...
      mockFetchPage.mockResolvedValueOnce({
        content: "",
        title: "",
        url: "https://www.newyorker.com/culture/john-wayne-profile",
        fetchMethod: "direct",
        error: "HTTP 401",
      })
    ...
    it("returns failure when cleaned content is too short", async () => {
      const encodedUrl = encodeURIComponent("https://www.newyorker.com/magazine/john-wayne-brief")
      ...
      mockFetchPage.mockResolvedValueOnce({
        ...
        title: "John Wayne",
        url: "https://www.newyorker.com/magazine/john-wayne-brief",
        fetchMethod: "direct",
      })
    ...
    it("sets correct source metadata on success", async () => {
      ...
      expect(result.success).toBe(true)
      expect(result.source.publication).toBe("The New Yorker")
      expect(result.source.domain).toBe("newyorker.com")
      expect(result.source.contentType).toBe("profile")
      expect(result.source.url).toContain("newyorker.com")
