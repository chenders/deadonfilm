...
export async function writeBiographyToProduction(
  ...
  await newrelic.startSegment("BioDB/writeProduction", true, async () => {
    ...
    if (actorResult.rows.length > 0) {
      ...
      if (biography && biography_legacy === null) {
        await db.query(`UPDATE actors SET biography_legacy = $1 WHERE id = $2`, [
          biography,
          actorId,
        ])
        archivedLegacy = true
    ...
    await db.query(
      ...
      ON CONFLICT (actor_id) DO UPDATE SET
        ...
        lesser_known_facts = EXCLUDED.lesser_known_facts,
