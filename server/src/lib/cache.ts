...
export async function invalidateActorCache(actorId: number): Promise<void> {
  ...
  // Invalidate prerendered and SSR-rendered pages for this specific actor
  ...
  await invalidateByPattern(`${CACHE_PREFIX.SSR}:*:/actor/*-${actorId}`)
}
...
export async function invalidateActorCacheRequired(actorId: number): Promise<void> {
  ...
  const keys = getActorCacheKeys(actorId)
  await instrumentedDel(...keys)
  // Also invalidate prerendered and SSR-rendered pages for this actor
  // Use exact pattern without trailing wildcard to prevent over-matching
  await invalidateByPattern(`${CACHE_PREFIX.PRERENDER}:*:/actor/*-${actorId}`)
  await invalidateByPattern(`${CACHE_PREFIX.SSR}:*:/actor/*-${actorId}`)
