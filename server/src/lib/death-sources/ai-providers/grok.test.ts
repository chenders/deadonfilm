...
import { GrokSource } from "./grok.js"
...
describe("GrokSource", () => {
  ...
  beforeEach(() => {
    vi.clearAllMocks()
    vi.stubEnv("XAI_API_KEY", "test-api-key")
    source = new GrokSource()
  })
  ...
  describe("properties", () => {
    it("has correct name", () => {
      expect(source.name).toBe("Grok")
    })

    it("has correct type", () => {
      expect(source.type).toBe(DataSourceType.GROK)
    })
    ...
    it("is not available without API key", () => {
      vi.unstubAllEnvs()
      const sourceWithoutKey = new GrokSource()
  ...
  describe("lookup", () => {
    ...
    it("returns source entry with correct type", async () => {
      ...
      expect(result.source.type).toBe(DataSourceType.GROK)
    })
  })
})
