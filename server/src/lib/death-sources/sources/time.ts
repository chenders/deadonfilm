/**
 * Time source for death/obituary news.
 *
 * Searches for Time articles via DDG/Google CSE (with site:time.com),
 * then fetches and parses the article for death information.
 * Falls back to archive.org if Time blocks direct access (401/403).
 *
 * Time magazine provides coverage of notable deaths across entertainment,
...
/**
 * Time source for actor death and obituary news.
 */
export class TimeSource extends BaseDataSource {
  readonly name = "Time"
  readonly type = DataSourceType.TIME_MAGAZINE
  ...
  protected async performLookup(actor: ActorForEnrichment): Promise<SourceLookupResult> {
    ...
    try {
      ...
      const searchQuery = `site:time.com "${actor.name}" obituary OR died OR death ${deathYear}`
      ...
      const articleUrl = this.extractTimeUrl(searchHtml, actor)
      ...
      if (!articleUrl) {
        return {
          ...
          source: this.createSourceEntry(startTime, 0),
          data: null,
          error: "No Time obituary found in search results",
        }
      }
      ...
      if (!articleData) {
        return {
          ...
          data: null,
          error: "Could not fetch Time article",
        }
      }
      ...
      if (!articleData.circumstances) {
        return {
          ...
          data: null,
          error: "No death information found in Time article",
        }
      }
  ...
  /**
   * Extract Time URL from search results.
   */
  private extractTimeUrl(html: string, actor: ActorForEnrichment): string | null {
    const urlPattern = /https?:\/\/(?:www\.)?time\.com\/[^"'\s<>]+/gi
    return extractUrlFromSearchResults(html, urlPattern, actor)
  }

  /**
   * Fetch and parse a Time article.
   * Falls back to archive.org if Time blocks direct access.
   */
