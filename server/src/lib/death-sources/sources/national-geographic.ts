/**
 * National Geographic source for death/obituary news.
 *
 * Searches for National Geographic articles via DDG/Google CSE
 * (with site:nationalgeographic.com), then fetches and parses the
 * article for death information. Falls back to archive.org if
 * National Geographic blocks direct access (401/403).
 *
 * National Geographic covers deaths of explorers, scientists,
 * conservationists, and documentary subjects.
 *
...
/**
 * National Geographic source for actor death and obituary news.
 */
export class NationalGeographicSource extends BaseDataSource {
  readonly name = "National Geographic"
  readonly type = DataSourceType.NATIONAL_GEOGRAPHIC
  ...
  protected async performLookup(actor: ActorForEnrichment): Promise<SourceLookupResult> {
    ...
    try {
      ...
      const searchQuery = `site:nationalgeographic.com "${actor.name}" obituary OR died OR death ${deathYear}`
      ...
      const articleUrl = this.extractNationalGeographicUrl(searchHtml, actor)
      ...
      if (!articleUrl) {
        return {
          ...
          source: this.createSourceEntry(startTime, 0),
          data: null,
          error: "No National Geographic obituary found in search results",
        }
      }
      ...
      if (!articleData) {
        return {
          ...
          data: null,
          error: "Could not fetch National Geographic article",
        }
      }
      ...
      if (!articleData.circumstances) {
        return {
          ...
          data: null,
          error: "No death information found in National Geographic article",
        }
      }
  ...
  /**
   * Extract National Geographic URL from search results.
   */
  private extractNationalGeographicUrl(html: string, actor: ActorForEnrichment): string | null {
    const urlPattern = /https?:\/\/(?:www\.)?nationalgeographic\.com\/[^"'\s<>]+/gi
    return extractUrlFromSearchResults(html, urlPattern, actor)
  }

  /**
   * Fetch and parse a National Geographic article.
   * Falls back to archive.org if National Geographic blocks direct access.
   */
  private async fetchArticle(url: string, actor: ActorForEnrichment): Promise<ArticleData | null> {
    ...
    if (response.status === 403 || response.status === 401) {
      console.log(
        `  National Geographic blocked (${response.status}), trying archive.org fallback...`
      )
      const archiveResult = await fetchFromArchive(url)
      if (archiveResult.success && archiveResult.content) {
        console.log(`  Archive.org fallback succeeded for National Geographic`)
        return this.parseArticle(archiveResult.content, actor)
      }
      throw new SourceAccessBlockedError(
        `National Geographic blocked access (${response.status})`,
        this.type,
        url,
        response.status
      )
