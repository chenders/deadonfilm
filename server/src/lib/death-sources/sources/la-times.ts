/**
 * LA Times source for death/obituary news.
 *
 * Searches for LA Times articles via DDG/Google CSE (with site:latimes.com),
 * then fetches and parses the article for death information.
 * Falls back to archive.org if LA Times blocks direct access (401/403).
 *
 * The Los Angeles Times provides extensive coverage of entertainment
 * industry deaths and obituaries.
 *
...
/**
 * LA Times source for actor death and obituary news.
 */
export class LATimesSource extends BaseDataSource {
  readonly name = "LA Times"
  readonly type = DataSourceType.LA_TIMES
  ...
  protected async performLookup(actor: ActorForEnrichment): Promise<SourceLookupResult> {
    ...
    try {
      ...
      const searchQuery = `site:latimes.com "${actor.name}" obituary OR died OR death ${deathYear}`
      ...
      const articleUrl = this.extractLATimesUrl(searchHtml, actor)
      ...
      if (!articleUrl) {
        return {
          ...
          source: this.createSourceEntry(startTime, 0),
          data: null,
          error: "No LA Times obituary found in search results",
        }
      }
      ...
      if (!articleData) {
        return {
          ...
          data: null,
          error: "Could not fetch LA Times article",
        }
      }
      ...
      if (!articleData.circumstances) {
        return {
          ...
          data: null,
          error: "No death information found in LA Times article",
        }
      }
  ...
  /**
   * Extract LA Times URL from search results.
   */
  private extractLATimesUrl(html: string, actor: ActorForEnrichment): string | null {
    const urlPattern = /https?:\/\/(?:www\.)?latimes\.com\/[^"'\s<>]+/gi
    return extractUrlFromSearchResults(html, urlPattern, actor)
  }

  /**
   * Fetch and parse an LA Times article.
   * Falls back to archive.org if LA Times blocks direct access.
   */
