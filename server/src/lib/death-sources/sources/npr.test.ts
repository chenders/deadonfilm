...
describe("NPRSource", () => {
  let source: NPRSource

  beforeEach(() => {
    source = new NPRSource()
  ...
  describe("properties", () => {
    ...
    it("has correct type", () => {
      expect(source.type).toBe(DataSourceType.NPR)
    })
    it("is free", () => {
  ...
  describe("lookup", () => {
    ...
    it("finds obituary and extracts death information", async () => {
      mockFetch.mockResolvedValueOnce({
        ...
        status: 200,
        text: async () =>
          `<html><body><a href="https://www.npr.org/sections/john-smith-obituary-2024">Obituary</a></body></html>`,
      })
      mockFetch.mockResolvedValueOnce({
        ok: true,
        status: 200,
      ...
      expect(result.source.type).toBe(DataSourceType.NPR)
    ...
    it("returns error when no obituary found", async () => {
      ...
      const result = await source.lookup(mockActor)
      expect(result.success).toBe(false)
      expect(result.error).toContain("No NPR obituary found")
    })
    ...
    it("tries archive.org fallback on 403", async () => {
      mockFetch.mockResolvedValueOnce({
        ...
        status: 200,
        text: async () =>
          `<html><body><a href="https://www.npr.org/sections/john-smith/">Article</a></body></html>`,
      })
    ...
    it("returns error when article fetch fails", async () => {
      ...
      const result = await source.lookup(mockActor)
      expect(result.success).toBe(false)
      expect(result.error).toBe("Could not fetch NPR article")
    })
