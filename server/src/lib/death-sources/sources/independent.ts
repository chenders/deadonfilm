/**
 * The Independent source for death/obituary news.
 *
 * Searches for Independent articles via DDG/Google CSE (with site:independent.co.uk),
 * then fetches and parses the article for death information.
 * Falls back to archive.org if The Independent blocks direct access (401/403).
 *
 * The Independent provides coverage of notable deaths and obituaries.
 *
...
/**
 * The Independent source for actor death and obituary news.
 */
export class IndependentSource extends BaseDataSource {
  readonly name = "The Independent"
  readonly type = DataSourceType.INDEPENDENT
  ...
  protected async performLookup(actor: ActorForEnrichment): Promise<SourceLookupResult> {
    ...
    try {
      ...
      const searchQuery = `site:independent.co.uk "${actor.name}" obituary OR died OR death ${deathYear}`
      ...
      const articleUrl = this.extractIndependentUrl(searchHtml, actor)
      ...
      if (!articleUrl) {
        return {
          ...
          source: this.createSourceEntry(startTime, 0),
          data: null,
          error: "No Independent obituary found in search results",
        }
      }
      ...
      if (!articleData) {
        return {
          ...
          data: null,
          error: "Could not fetch Independent article",
        }
      }
      ...
      if (!articleData.circumstances) {
        return {
          ...
          data: null,
          error: "No death information found in Independent article",
        }
      }
  ...
  /**
   * Extract Independent URL from search results.
   */
  private extractIndependentUrl(html: string, actor: ActorForEnrichment): string | null {
    const urlPattern = /https?:\/\/(?:www\.)?independent\.co\.uk\/[^"'\s<>]+/gi
    return extractUrlFromSearchResults(html, urlPattern, actor)
  }

  /**
   * Fetch and parse an Independent article.
   * Falls back to archive.org if The Independent blocks direct access.
   */
  private async fetchArticle(url: string, actor: ActorForEnrichment): Promise<ArticleData | null> {
    ...
    if (response.status === 403 || response.status === 401) {
      console.log(`  The Independent blocked (${response.status}), trying archive.org fallback...`)
      const archiveResult = await fetchFromArchive(url)
      if (archiveResult.success && archiveResult.content) {
        console.log(`  Archive.org fallback succeeded for The Independent`)
        return this.parseArticle(archiveResult.content, actor)
      }
      throw new SourceAccessBlockedError(
        `The Independent blocked access (${response.status})`,
        this.type,
        url,
        response.status
      )
