/**
 * The Telegraph source for death/obituary news.
 *
 * Searches for Telegraph articles via DDG/Google CSE (with site:telegraph.co.uk),
 * then fetches and parses the article for death information.
 * Falls back to archive.org if The Telegraph blocks direct access (401/403).
 *
 * The Daily Telegraph provides detailed obituaries of notable public figures.
 *
...
/**
 * The Telegraph source for actor death and obituary news.
 */
export class TelegraphSource extends BaseDataSource {
  readonly name = "The Telegraph"
  readonly type = DataSourceType.TELEGRAPH
  ...
  protected async performLookup(actor: ActorForEnrichment): Promise<SourceLookupResult> {
    ...
    try {
      ...
      const searchQuery = `site:telegraph.co.uk "${actor.name}" obituary OR died OR death ${deathYear}`
      ...
      const articleUrl = this.extractTelegraphUrl(searchHtml, actor)
      ...
      if (!articleUrl) {
        return {
          ...
          source: this.createSourceEntry(startTime, 0),
          data: null,
          error: "No Telegraph obituary found in search results",
        }
      }
      ...
      if (!articleData) {
        return {
          ...
          data: null,
          error: "Could not fetch Telegraph article",
        }
      }
      ...
      if (!articleData.circumstances) {
        return {
          ...
          data: null,
          error: "No death information found in Telegraph article",
        }
      }
  ...
  /**
   * Extract Telegraph URL from search results.
   */
  private extractTelegraphUrl(html: string, actor: ActorForEnrichment): string | null {
    const urlPattern = /https?:\/\/(?:www\.)?telegraph\.co\.uk\/[^"'\s<>]+/gi
    return extractUrlFromSearchResults(html, urlPattern, actor)
  }

  /**
   * Fetch and parse a Telegraph article.
   * Falls back to archive.org if The Telegraph blocks direct access.
   */
  private async fetchArticle(url: string, actor: ActorForEnrichment): Promise<ArticleData | null> {
    ...
    if (response.status === 403 || response.status === 401) {
      console.log(`  The Telegraph blocked (${response.status}), trying archive.org fallback...`)
      const archiveResult = await fetchFromArchive(url)
      if (archiveResult.success && archiveResult.content) {
        console.log(`  Archive.org fallback succeeded for The Telegraph`)
        return this.parseArticle(archiveResult.content, actor)
      }
      throw new SourceAccessBlockedError(
        `The Telegraph blocked access (${response.status})`,
        this.type,
        url,
        response.status
      )
