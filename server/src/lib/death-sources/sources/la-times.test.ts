...
import { LATimesSource } from "./la-times.js"
...
describe("LATimesSource", () => {
  let source: LATimesSource

  beforeEach(() => {
    source = new LATimesSource()
  ...
  describe("properties", () => {
    it("has correct name", () => {
      expect(source.name).toBe("LA Times")
    })
    it("has correct type", () => {
      expect(source.type).toBe(DataSourceType.LA_TIMES)
    })
    it("is free", () => {
  ...
  describe("lookup", () => {
    ...
    it("finds obituary and extracts death information", async () => {
      mockFetch.mockResolvedValueOnce({
        ...
        status: 200,
        text: async () =>
          `<html><body><a href="https://www.latimes.com/obituaries/john-smith-2024">Obituary</a></body></html>`,
      })
      mockFetch.mockResolvedValueOnce({
        ok: true,
        status: 200,
        text: async () =>
          `<html><body><p>John Smith, the actor, died on June 1, 2024 at his home in Los Angeles after a battle with cancer.</p></body></html>`,
      })
      const result = await source.lookup(mockActor)
      expect(result.success).toBe(true)
      expect(result.source.type).toBe(DataSourceType.LA_TIMES)
    ...
    it("returns error when no obituary found", async () => {
      ...
      const result = await source.lookup(mockActor)
      expect(result.success).toBe(false)
      expect(result.error).toContain("No LA Times obituary found")
    })
    ...
    it("tries archive.org fallback on 403", async () => {
      mockFetch.mockResolvedValueOnce({
        ...
        status: 200,
        text: async () =>
          `<html><body><a href="https://www.latimes.com/obituaries/john-smith/">Article</a></body></html>`,
      })
    ...
    it("returns error when article has no death info", async () => {
      mockFetch.mockResolvedValueOnce({
        ...
        status: 200,
        text: async () =>
          `<html><body><a href="https://www.latimes.com/entertainment/john-smith/">Article</a></body></html>`,
      })
      mockFetch.mockResolvedValueOnce({
        ok: true,
        status: 200,
    ...
    it("returns error when article fetch fails", async () => {
      ...
      const result = await source.lookup(mockActor)
      expect(result.success).toBe(false)
      expect(result.error).toBe("Could not fetch LA Times article")
    })
