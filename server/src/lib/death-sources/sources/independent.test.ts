...
import { IndependentSource } from "./independent.js"
...
describe("IndependentSource", () => {
  let source: IndependentSource

  beforeEach(() => {
    source = new IndependentSource()
  ...
  describe("properties", () => {
    ...
    it("has correct type", () => {
      expect(source.type).toBe(DataSourceType.INDEPENDENT)
    })
    it("is free", () => {
  ...
  describe("lookup", () => {
    ...
    it("finds obituary and extracts death information", async () => {
      mockFetch.mockResolvedValueOnce({
        ...
        status: 200,
        text: async () =>
          `<html><body><a href="https://www.independent.co.uk/news/john-smith-obituary-2024">Obituary</a></body></html>`,
      })
      mockFetch.mockResolvedValueOnce({
        ok: true,
        status: 200,
      ...
      expect(result.source.type).toBe(DataSourceType.INDEPENDENT)
    ...
    it("returns error when no obituary found", async () => {
      ...
      const result = await source.lookup(mockActor)
      expect(result.success).toBe(false)
      expect(result.error).toContain("No Independent obituary found")
    })
    ...
    it("tries archive.org fallback on 403", async () => {
      mockFetch.mockResolvedValueOnce({
        ...
        status: 200,
        text: async () =>
          `<html><body><a href="https://www.independent.co.uk/news/john-smith/">Article</a></body></html>`,
      })
    ...
    it("returns error when article fetch fails", async () => {
      ...
      const result = await source.lookup(mockActor)
      expect(result.success).toBe(false)
      expect(result.error).toBe("Could not fetch Independent article")
    })
