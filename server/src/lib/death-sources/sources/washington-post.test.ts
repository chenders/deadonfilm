...
import { WashingtonPostSource } from "./washington-post.js"
...
describe("WashingtonPostSource", () => {
  let source: WashingtonPostSource

  beforeEach(() => {
    source = new WashingtonPostSource()
  ...
  describe("properties", () => {
    ...
    it("has correct type", () => {
      expect(source.type).toBe(DataSourceType.WASHINGTON_POST)
    })
  ...
  describe("lookup", () => {
    ...
    it("finds obituary and extracts death information", async () => {
      // Search returns WaPo URL
      const searchHtml = `
        <html><body>
          <a href="https://www.washingtonpost.com/obituaries/john-smith-death-2024-06-01">
      ...
      expect(result.source.type).toBe(DataSourceType.WASHINGTON_POST)
    ...
    it("returns error when no obituary found in search", async () => {
      ...
      expect(result.success).toBe(false)
      expect(result.error).toContain("No Washington Post obituary found")
    })
    ...
    it("tries archive.org fallback on 401 during article fetch", async () => {
      ...
      const searchHtml = `
        <html><body>
          <a href="https://www.washingtonpost.com/obituaries/john-smith-dead/">Article</a>
        </body></html>
      `
      mockFetch.mockResolvedValueOnce({
        ok: true,
      ...
      vi.mocked(fetchFromArchive).mockResolvedValueOnce({
        success: false,
        url: "https://www.washingtonpost.com/obituaries/john-smith-dead/",
        archiveUrl: null,
        title: "",
        content: "",
        contentLength: 0,
      ...
      expect(fetchFromArchive).toHaveBeenCalledWith(
        "https://www.washingtonpost.com/obituaries/john-smith-dead/"
      )
    })
    ...
    it("uses archive.org content when direct access is blocked", async () => {
      ...
      const searchHtml = `
        <html><body>
          <a href="https://www.washingtonpost.com/entertainment/john-smith-dies/">Obituary</a>
        </body></html>
      `
      mockFetch.mockResolvedValueOnce({
        ok: true,
      ...
      vi.mocked(fetchFromArchive).mockResolvedValueOnce({
        success: true,
        url: "https://www.washingtonpost.com/entertainment/john-smith-dies/",
        archiveUrl:
          "https://web.archive.org/web/20240601/https://www.washingtonpost.com/entertainment/john-smith-dies/",
    ...
    it("throws SourceAccessBlockedError on 403 when archive also fails", async () => {
      const searchHtml = `
        <html><body>
          <a href="https://www.washingtonpost.com/nation/john-smith-dead/">Article</a>
        </body></html>
      `
      mockFetch.mockResolvedValueOnce({
        ok: true,
    ...
    it("returns error when article fetch fails with non-blocking status", async () => {
      ...
      expect(result.success).toBe(false)
      expect(result.error).toBe("Could not fetch Washington Post article")
    })

    it("returns error when article has no death info", async () => {
      const searchHtml = `
        <html><body>
          <a href="https://www.washingtonpost.com/entertainment/john-smith-interview/">Article</a>
        </body></html>
      `
      mockFetch.mockResolvedValueOnce({
        ok: true,
    ...
    it("extracts location of death from article", async () => {
      const searchHtml = `
        <html><body>
          <a href="https://www.washingtonpost.com/obituaries/john-smith-dies/">Obituary</a>
        </body></html>
      `
      mockFetch.mockResolvedValueOnce({
        ok: true,
    ...
    it("matches various WaPo URL patterns", async () => {
      // WaPo uses /obituaries/, /entertainment/, /style/, /nation/, etc.
      const searchHtml = `
        <html><body>
          <a href="https://www.washingtonpost.com/style/john-smith-obituary-2024-06-01">Obituary</a>
        </body></html>
      `
      mockFetch.mockResolvedValueOnce({
        ok: true,
