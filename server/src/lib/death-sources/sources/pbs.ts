/**
 * PBS source for death/obituary news.
 *
 * Searches for PBS articles via DDG/Google CSE (with site:pbs.org),
 * then fetches and parses the article for death information.
 * Falls back to archive.org if PBS blocks direct access (401/403).
 *
 * PBS provides in-depth coverage of notable deaths spanning documentary
 * subjects, scientists, authors, and public figures.
 *
...
/**
 * PBS source for actor death and obituary news.
 */
export class PBSSource extends BaseDataSource {
  readonly name = "PBS"
  readonly type = DataSourceType.PBS
  ...
  protected async performLookup(actor: ActorForEnrichment): Promise<SourceLookupResult> {
    ...
    try {
      ...
      const searchQuery = `site:pbs.org "${actor.name}" obituary OR died OR death ${deathYear}`
      ...
      const articleUrl = this.extractPBSUrl(searchHtml, actor)
      ...
      if (!articleUrl) {
        return {
          ...
          source: this.createSourceEntry(startTime, 0),
          data: null,
          error: "No PBS obituary found in search results",
        }
      }
      ...
      if (!articleData) {
        return {
          ...
          data: null,
          error: "Could not fetch PBS article",
        }
      }
      ...
      if (!articleData.circumstances) {
        return {
          ...
          data: null,
          error: "No death information found in PBS article",
        }
      }
  ...
  /**
   * Extract PBS URL from search results.
   */
  private extractPBSUrl(html: string, actor: ActorForEnrichment): string | null {
    const urlPattern = /https?:\/\/(?:www\.)?pbs\.org\/[^"'\s<>]+/gi
    return extractUrlFromSearchResults(html, urlPattern, actor)
  }

  /**
   * Fetch and parse a PBS article.
   * Falls back to archive.org if PBS blocks direct access.
   */
