...
import { TelegraphSource } from "./telegraph.js"
...
describe("TelegraphSource", () => {
  let source: TelegraphSource

  beforeEach(() => {
    source = new TelegraphSource()
  ...
  describe("properties", () => {
    ...
    it("has correct type", () => {
      expect(source.type).toBe(DataSourceType.TELEGRAPH)
    })
    it("is free", () => {
  ...
  describe("lookup", () => {
    ...
    it("finds obituary and extracts death information", async () => {
      mockFetch.mockResolvedValueOnce({
        ...
        status: 200,
        text: async () =>
          `<html><body><a href="https://www.telegraph.co.uk/obituaries/john-smith-obituary-2024">Obituary</a></body></html>`,
      })
      mockFetch.mockResolvedValueOnce({
        ok: true,
        status: 200,
      ...
      expect(result.source.type).toBe(DataSourceType.TELEGRAPH)
    ...
    it("returns error when no obituary found", async () => {
      ...
      const result = await source.lookup(mockActor)
      expect(result.success).toBe(false)
      expect(result.error).toContain("No Telegraph obituary found")
    })
    ...
    it("tries archive.org fallback on 403", async () => {
      mockFetch.mockResolvedValueOnce({
        ...
        status: 200,
        text: async () =>
          `<html><body><a href="https://www.telegraph.co.uk/obituaries/john-smith/">Article</a></body></html>`,
      })
    ...
    it("returns error when article fetch fails", async () => {
      ...
      const result = await source.lookup(mockActor)
      expect(result.success).toBe(false)
      expect(result.error).toBe("Could not fetch Telegraph article")
    })
