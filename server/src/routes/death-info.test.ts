...
describe("getDeathInfoRoute", () => {
  ...
  beforeEach(() => {
    vi.clearAllMocks()
    // Set DATABASE_URL so getActorsByInternalIds calls the mocked function
  ...
  it("returns death info for valid request", async () => {
    ...
    vi.mocked(getActorsByInternalIds).mockResolvedValue(mockDbRecords as any)
    ...
    expect(getActorsByInternalIds).toHaveBeenCalledWith([123, 456])
    expect(jsonSpy).toHaveBeenCalledWith({
      pending: false,
      deathInfo: {
        123: {
  ...
  it("handles empty results", async () => {
    vi.mocked(getActorsByInternalIds).mockResolvedValue(new Map())
  ...
  it("handles single person ID", async () => {
    ...
    expect(getActorsByInternalIds).toHaveBeenCalledWith([123])
    expect(jsonSpy).toHaveBeenCalledWith({
      pending: false,
      deathInfo: {
        123: {
